...
<div class="mentor">
  ...
  {:else if data}
    ...
    {#if data.can_manage&&!demo}<details class="parents"><summary>Echte Einstellungen und Übungen verwalten</summary>
      ...
      <p><strong>{data.budget.used_eur.toFixed(2)} € von {data.budget.limit_eur.toFixed(0)} €</strong> · {data.budget.month}</p><p class="muted">{data.budget.accounting}. Warnung ab 40 €. Hintergrundarbeiten sind enthalten{#if data.budget.sources_limit_eur}, darunter der Quellenbestand mit {data.budget.sources_eur.toFixed(2)} € von {data.budget.sources_limit_eur.toFixed(0)} €{/if}.</p>
