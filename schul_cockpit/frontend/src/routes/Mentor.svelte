...
<div class="mentor">
  ...
  {:else if data}
    ...
    {#if tab==='today'}
      {#if !data.can_manage||demo}<section class="card material-hint"><h2>Hast du ein Blatt oder eine Heftseite?</h2><p>Fotografiere es, dann kann ich beim Üben und beim Test daraus arbeiten.</p><a href={`#/materialien/${subject||''}`}><ActionLabel label="📎 Blatt oder Heftseite hinzufügen" /></a></section>
