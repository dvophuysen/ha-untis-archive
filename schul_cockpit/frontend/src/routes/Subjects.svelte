...
{:else}
  ...
  {#each subjects as s (s.subject_id)}
    ...
    <section class="card subject-card">
      <header class="subject-head"><h2>{subjectStyle(s.name).emoji} {subjectStyle(s.name).name}</h2><button class="ghost" onclick={()=>navigate('subject',s.subject_id)} aria-label={`Stundenverlauf ${s.name}`}>Verlauf →</button></header>
      <div class="subject-state">
        ...
        {:else if group?.feedback_count}<strong>🌿 Zuletzt keine Fragen gemeldet</strong>
        {:else}<span>📝 Noch keine Rückmeldung</span>{/if}
      ...
      {#each group?.items || [] as it (it.lesson_id)}
        <article class="topic">
          <h3>{it.lstext}</h3>
          <div class="topic-actions"><span class="small dim">{it.source_count} {it.source_count===1?'Stunde':'Stunden'} · zuletzt {formatShortDate(it.date)}</span><a href={it.url}>💬 Üben →</a></div>
