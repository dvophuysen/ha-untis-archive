<script>
 ...
 let {accountId}=$props();
 let data=$state(null),time=$state(''),enabled=$state(false),morning=$state(''),morningOn=$state(false),busy=$state(false),error=$state(''),message=$state(''),device=$state(false),supported=$state(false);
 let request=0;
