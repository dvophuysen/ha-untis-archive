  ...
  {#if canManage}<button disabled={busy} onclick={()=>removeConfirm=!removeConfirm}>Diese Einheit entfernen</button>{#if removeConfirm}<p class="notice">Gespräch, Antworten und Anrechnung dieser Einheit löschen?</p><button disabled={busy} onclick={()=>act(removeSession)}>Einheit endgültig löschen</button>{/if}{/if}
    ...
    <div class="messages" aria-live="polite">
      {#each running.messages as m, i}<article class:own={m.role==='user'}><span class="speaker">{m.role!=='user'?'Mentor':m.author==='eltern'?'Eltern':canManage?'Kind':'Du'}</span><p class="preserve">{m.text}</p>
        ...
        {#if m.payload.task}<div class="task"><strong>Deine Aufgabe</strong>{#if m.payload.task.vorlage}<blockquote class="vorlage preserve">{m.payload.task.vorlage}{#if m.payload.task.quelle}<cite>{m.payload.task.quelle}</cite>{/if}</blockquote>{/if}{#if m.payload.task.figur_src}<button type="button" class="figure" onclick={()=>zoom={src:m.payload.task.figur_src}}><img src={m.payload.task.figur_src} alt={m.payload.task.figur_text||'Abbildung zur Aufgabe'}/><small>antippen zum Vergrößern</small></button>{/if}{#if m.payload.task.abbildung}<button type="button" class="figure" onclick={()=>zoom=m.payload.task.abbildung}><img src={figUrl(m.payload.task.abbildung)} alt={m.payload.task.abbildung_text||'Abbildung zur Aufgabe'} loading="lazy"/><small>{m.payload.task.abbildung_seite||'Abbildung'} · antippen zum Vergrößern</small></button>{/if}<p class="preserve">{m.payload.task.prompt}</p>{#if m.payload.task.optionen?.length}<div class="optionen" role="group" aria-label="Antwortmöglichkeiten">{#each m.payload.task.optionen as o (o.id)}<button class="option" class:aus={o.aus} disabled={busy||o.aus||!canWrite||i!==liveTask||running.status!=='active'} onclick={()=>act(()=>send('choice',o.text,o.id))}>{o.text}</button>{/each}</div>{/if}</div>{/if}
    ...
    {#if zoom}<div class="zoom" role="dialog" aria-modal="true" aria-label="Abbildung vergrößert"><button type="button" class="primary" onclick={()=>zoom=null}>Schließen</button><img src={zoom?.src??figUrl(zoom)} alt="Abbildung vergrößert"/></div>{/if}
