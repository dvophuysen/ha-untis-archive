...
- Entschieden (D219, 1.41.0): „sicher“ erst nach zwei Aufgaben, auch beim vollen Einstiegstest; Gesprächsaufgaben werden nachgerechnet (zweiter Versuch mit Befund, sonst keine Aufgabe), beim Bewerten zählt das Richtige. Rechnerprüfung liest auch Bruch-Rechenketten (`arith_issues`); Prüfkorpus: 117 Aufgaben aus Lesezugang (`learning_activities`) und Übungsarbeiten, 0 Fehlalarme, 29 % der verfälschten Ergebnisse erkannt. Wer den Leser erweitert, zuerst gegen einen solchen Korpus laufen lassen: Jeder Fehlalarm hält bei Übungsarbeiten die Ausgabe auf.
...
- Vokabeln (1.38.0, D212): Haltbarkeits-Modell statt fester Schritte, Rangfolge im Trainer, Grundpensum aus aktiven Einheiten (höchstens 15). Stellschrauben stehen gebündelt oben in `vocab.py` (`FIRST_RIGHT_DAYS`, `GROW`, `SHRINK`, `RECALL_DUE`, `SURE_DAYS`, `FIRM_DAYS`, `NEW_PRIORITY`) und in `vocab_pensum.py` (`BASE_MAX`, `LESSON_DAYS`, `UNSCOPED_MIN`); der Stand wird bei jedem Lesen aus allen Antworten neu gerechnet, eine Änderung wirkt also sofort und rückwirkend.
- **Wiedervorlage Eichung (ab 10.10.2026):** `python3 scripts/vocab_calibration.py` laufen lassen und beurteilen, ob die Kinder mit dem Stoff durchkommen (Nutzer: „du musst sehen, wie wir mit dem Stoff durchkommen“). Wächst der Rückstau fälliger Wörter Woche für Woche, `BASE_MAX` erhöhen oder `GROW` anheben; liegt die Trefferquote nach Pausen deutlich über 90 %, wächst die Haltbarkeit zu langsam (`GROW` oder `FIRST_RIGHT_DAYS` höher), unter etwa 70 % zu schnell. Ausgangswerte am 26.09. (Antworten bis 25.09.): Treffer beim ersten Versuch 47 % und 37 %, nach einer Pause 93 % und 92 %; Haltbarkeit der sicheren Wörter im Median 3 Tage. Jede Änderung mit Zahlen in ENTSCHEIDUNGEN.md unter D212 nachtragen und dem Nutzer vorlegen. Die Zögerzeit bleibt draußen, bis der Nutzer anders entscheidet.
