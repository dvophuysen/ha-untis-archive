...
def compute(account_id: int, day: date) -> list[dict]:
    ...
    # Die nächste Arbeit zuerst, spätere danach (D215, ersetzt den Wechsel aus
    # D189). Im Notfall nur die Arbeiten im Notfall (D213): eine unsichere
    # Arbeit in den nächsten zwei Schultagen oder eine, bei der es eng wird;
    # dazu ein Vokabeltest im selben Fenster.
    groups = [(p["exam_date"], p["steps"]) for p in ordered if p["steps"] and (not focus or p["exam_key"] in focus)]
    groups += [(v.get("exam_date") or "9999-12-31", [_vocab_step(v)]) for v in vocab
               if v.get("exam_key") and (not focus or _vocab_in_window(account_id, day, v))]
    groups.sort(key=lambda g: g[0])
    steps = [st for _, g in groups for st in g]
    if steps:
        return steps
...
def _focus(account_id: int, day: date, ordered: list[dict]) -> set[str]:
    """Arbeiten im Notfall, für die heute etwas zu tun ist: in den nächsten zwei
    Schultagen (D213) oder mit Engpass, wo die Lerntage nicht mehr reichen (D215)."""
    until = _focus_until(account_id, day)
    return {p["exam_key"] for p in ordered if p["need"] and p["steps"]
            and ((until is not None and p["exam_date"] <= until.isoformat()) or p.get("weekend") or p.get("behind"))}
...
def view(account_id: int, day: date, *, store: bool, until: date | None = None) -> dict:
    ...
    # Nächste Arbeit zuerst (D215), auch in einer schon festgehaltenen Liste:
    # nur die Reihenfolge, nichts kommt dazu oder fällt weg.
    checked.sort(key=lambda st: st.get("exam_date") or "9999-12-31")
    try:
