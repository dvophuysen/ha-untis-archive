...
def note_extra_practice(account_id: int, attempt_id: int, exam_key: str | None, user, day: date | None = None) -> bool:
    """Eine Übungsarbeit ohne Pflicht: an einem Tag ohne Unterricht (D179), bei
    einer Arbeit weiter als sechs Wochen weg oder ohne Papier-Schritt im Plan (D180)."""
    from . import rewards, vocab_pensum
    if not rewards.acting_child(user):
        return False
    day = day or _today()
    try:
        ...
        # Stand der Lernplan für diese Arbeit heute keine Übungsarbeit vor, ist sie freiwillig (D180).
        from . import study_plan
        planned = any(st.get("kind") == "paper" and st.get("exam_key") == exam_key
                      for st in (study_plan.stored(account_id, day) or []))
        if free or far or not planned:
