...
def sheet(exam,tasks,code=None):
    ...
    </style></head><body><div class="tools"><button onclick="window.print()">Drucken / als PDF sichern</button><p>Ohne Lösungen. Auf dem iPhone auch über Teilen → Drucken. {'Danach zur App zurückkehren und alle beschriebenen Seiten bei dieser Übungsarbeit fotografieren.' if code else 'Danach zur App zurückkehren und unter „Online / Foto bearbeiten“ die Fotos bei der jeweiligen Aufgabe einreichen.'}</p></div>
    <h1>{title}</h1>{('<p><strong>Blatt ' + escape(code) + '</strong></p>') if code else ''}<p>{escape(exam['subject'])} · {exam['minutes']} Minuten · {sum(t['points'] for t in tasks)} Punkte</p><p>Name: ____________________ Datum: ______________</p><p>{'Schreibe die Aufgabennummer auf jedes zusätzliche Blatt. Fotografiere am Ende alle Seiten gut lesbar, gerade von oben, bei hellem Licht.' if code else 'Schreibe die Aufgabennummer auf jedes zusätzliche Blatt. Fotografiere Antworten gut lesbar und ordne die Fotos in der App den Aufgaben zu.'}</p>{''.join(sections)}<footer>Übungsklausur · keine echte Schulnote · Aufgabenstand {exam['id']}{(' · Blatt ' + escape(code)) if code else ''}</footer></body></html>'''
