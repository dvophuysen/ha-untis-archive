...
await page.route('**/api/**',async route=>{const req=route.request(),u=new URL(req.url()).pathname;let body={};
 ...
 else if(u.endsWith('/today'))body={date:'2026-09-26',lessons:[],carry_lessons:{date:'2026-09-25',lessons:friday},next:{date:'2026-09-28',lessons:[{id:40,subject_name:'Deutsch',start_hhmm:'08:00',end_time:845,room:'1'}]},
...
// Die Doppelstunde zählt einmal und ist bewertet, Englisch ist offen: 1 von 2.
assert.equal(await ring.getAttribute('aria-label'),'Feedback: 1 von 2');
...
console.log('PASS: weekend rings show the Friday state, Friday lessons can still be rated');
