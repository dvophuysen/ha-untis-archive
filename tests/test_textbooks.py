...
def test_credentials_are_parent_only_and_validate_portal(env):
    for client, state in setup(env):
        for bad in ("http://beispiel-iserv.de", "https://evil.example", "https://user:pw@beispiel-iserv.de"):
            assert client.put(URL, json={"portal_url": bad, "username": "x", "password": "y"}).status_code == 422
        child(state)
        assert client.get(URL).status_code == 403
        assert client.put(URL, json={
            "portal_url": "https://beispiel-iserv.de", "username": "x", "password": "y"
        }).status_code == 403
...
def test_parent_can_verify_saved_access_without_exposing_secret(env):
    for client, _ in setup(env):
        assert client.put(URL, json={
            "portal_url": "https://beispiel-iserv.de", "username": "kind_a", "password": "secret"
        }).status_code == 200
        seen = {}
        ...
        assert seen == {"portal": "https://beispiel-iserv.de", "username": "kind_a", "password": "secret"}
