...
assert.equal(await a.getByText('1 Aufgabe überfällig',{exact:true}).count(),1);assert.equal(await a.getAttribute('data-status'),'problem');assert.equal(await b.getAttribute('data-status'),'good');assert.equal(await page.getByText('Was zeigt diese Übersicht?',{exact:true}).count(),0);
...
if(process.env.SCHOOL_SCREENSHOT_DIR){await page.setViewportSize({width:1024,height:1000});await page.screenshot({path:path.join(process.env.SCHOOL_SCREENSHOT_DIR,'family-redesign.png'),fullPage:true});}
