...
(async()=>{
 ...
 const page=await browser.newPage({viewport:{width:390,height:844},serviceWorkers:'block'});const errors=[];page.on('pageerror',e=>{errors.push(e.message);console.log('PAGE ERROR',e.message);});page.setDefaultTimeout(8000);let reminder={enabled:false,remind_at:null,devices:0},reminderWrites=0;let parent=false,created=false,published=false,sessionBody=null,deleted=false,failDelete=true;
 ...
 else if(u.endsWith('/reminders')){if(req.method()==='PUT'){reminder=req.postDataJSON();reminderWrites++;}body={...reminder,can_manage:parent,devices:0};}
 ...
 await page.goto('http://127.0.0.1:4179/#/settings');const reminders=page.locator('.reminder-settings');await reminders.getByText('Erinnerungen',{exact:false}).first().waitFor();assert.equal(reminderWrites,0);await reminders.getByLabel('Uhrzeit (Deutschland)').fill('18:00');await reminders.getByLabel('Tägliche Erinnerung einschalten').check();await reminders.getByRole('button',{name:'Speichern',exact:true}).click();await reminders.getByText('Erinnerungszeit gespeichert.',{exact:true}).waitFor();assert.equal(reminder.remind_at,'18:00');assert.equal(reminder.enabled,true);parent=false;await page.reload();await page.getByText(/Dein Tagescheck ist um 18:00/).waitFor();assert.equal(await reminders.getByRole('button',{name:'Speichern',exact:true}).count(),0);
