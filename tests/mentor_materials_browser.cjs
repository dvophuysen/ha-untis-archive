...
(async()=>{
 ...
 const subjects=[{name:'Deutsch',count:3},{name:'Mathe',count:1}];const shown=new Set();let uploads=0,subjectAsked=null,chosen=[],version=1,added=null,turn=null,dropped=null;
 const session=()=>({id:7,version,subject:'Deutsch',goal:'Hilfe: Aufgabe 4',label:'Aufgabe 4',mode:'homework_help',task_id:42,untimed:true,status:'active',messages:[{id:1,role:'assistant',text:'Wobei hängst du?',payload:{choices:[]},author:null}].concat(turn?[{id:2,role:'user',text:'Meine Seiten ansehen',payload:{material_ids:turn.ids},author:'kind'}]:[]),attachments:[],quiz:[],quiz_open:[],
   materials:chosen.map(id=>{const x=items.find(y=>y.id===id);return {id,title:x.title,label:x.label,mime_type:x.mime_type,date:x.date,shown:shown.has(id)};})});
 await page.route('**/api/**',async route=>{const req=route.request(),u=new URL(req.url()).pathname;let body={},status=200;
  ...
  else if(/\/sessions\/7\/materials\/\d+$/.test(u)){dropped=Number(u.split('/').pop());shown.delete(dropped);chosen=chosen.filter(x=>x!==dropped);version++;body=session();}
  else if(u.endsWith('/sessions/7/turn')){const b=req.postDataJSON();assert.equal(b.version,version);turn={ids:chosen.filter(id=>!shown.has(id)),text:b.text};turn.ids.forEach(id=>shown.add(id));version++;body=session();}
  else if(u.endsWith('/sessions/7/uploads')){uploads=(req.postData()||'').split('name="files"').length-1;for(let i=0;i<uploads;i++){const id=31+i;items.push({id,title:`Foto ${i+1}`,label:'',mime_type:'image/jpeg',date:'2026-09-22',reason:'',group:'linked',chosen:false});chosen.push(id);}version++;body=session();}
 ...
 // Angekreuzt und gleich gesendet, ohne „einbinden": Die Seite geht mit.
 await page.getByRole('checkbox',{name:/Heft S\. 13/}).check();
 await page.getByRole('button',{name:'Senden mit 1 Seite',exact:true}).click();
 await page.getByText('1 Seite aus deinen Materialien eingebunden',{exact:true}).waitFor();assert.deepEqual(added,[13]);
 // Mehrere Fotos aus der Hausaufgabe: Material der Aufgabe, sofort eingebunden.
 await page.locator('input.file').setInputFiles([{name:'a.png',mimeType:'image/png',buffer:png},{name:'b.png',mimeType:'image/png',buffer:png}]);
 await page.getByText(/Eingebunden: 4 Seiten · 2 neu/).waitFor();assert.equal(uploads,2);assert(!(await sendButton.isDisabled()));
