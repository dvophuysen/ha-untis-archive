...
Gegen Kind B (`kind.b`, Klasse 5E, Schule) end-to-end getestet:
...
denselben Modulen. Test-Creds via `PWD_kind_b` aus dem Container-Env.

## Noch offen / nicht erledigt

1. **Test gegen Kind A** (`kind.a`, andere Stufe?) — `PWD_KIND_A`
   war im letzten Session-Container nicht gesetzt. In der neuen Session
   sollte das funktionieren. Probe-Script: `/tmp/probe_kind_a.py` aus der
   Vorsession ist weg, aber das Muster ist `scripts/backfill.py` mit
   `UNTIS_USER=kind.a` und `UNTIS_PASS="$PWD_KIND_A"`. Nützlich
...
2. **Live-Deployment in HA** — Repo ist HACS-ready, aber noch nicht in
   ...
   - Pro Kind einen Config-Eintrag (Server `beispiel-gymnasium.webuntis.com`, Schule `beispiel-gymnasium`)
...
UNTIS_SERVER=beispiel-gymnasium.webuntis.com \
UNTIS_SCHOOL=beispiel-gymnasium \
UNTIS_USER=kind.b \
UNTIS_PASS="$PWD_kind_b" \
...
## Schul-spezifische Eigenheiten (Schule)
...
- **Klassenarbeiten** sind im `exam`-Feld von period/info enthalten —
  bei Kind B im aktuellen Fenster keine. Wenn welche kommen, landen sie
...
- User-E-Mail: `39620541+dvophuysen@users.noreply.github.com`
- Branch-Konvention vom Harness: `claude/<dreamy-curie-7iRxG>` ist das
  Arbeits-Branch dieser Sessions-Familie. PRs noch keine.
- `PWD_kind_b` / `PWD_KIND_A` sind als Container-Env-Vars für Tests gedacht.
