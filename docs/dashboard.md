...
Beispiel für Anzeigename `Kind B` (Device-Slug `untis_archive_kind_b`):

| Entity-ID | Inhalt |
|---|---|
| `calendar.untis_archive_kind_b_stundenplan` | Stundenplan-Kalender |
| `sensor.untis_archive_kind_b_lehrstoff_heute` | Heutige Stunden mit Lehrstoff |
| `sensor.untis_archive_kind_b_hausaufgaben_offen` | Offene Hausaufgaben |
| `sensor.untis_archive_kind_b_versaeumter_stoff` | Versäumter Stoff (14 Tage) |
| `sensor.untis_archive_kind_b_fehlzeiten_schuljahr` | Fehlzeiten im Schuljahr (mit `unexcused_count`) |
| `sensor.untis_archive_kind_b_stundenplan_aenderungen` | Vertretungen/Ausfälle/Lehrstoff-Updates (7 Tage) |
| `sensor.untis_archive_kind_b_fach_verlauf` | Lehrstoff-Verlauf gruppiert pro Fach |

Heißen deine Config-Entries anders, ersetze in `schul-cockpit.yaml`
`untis_archive_kind_b` / `untis_archive_kind_a` durch die tatsächlichen
...
- Name: `Schul-Cockpit – Kind`
  ...
  Optionen: `Kind B`, `Kind A` (genau wie die Untis-Anzeigenamen)
...
- Kind-Chip oben wechselt zwischen Kind B und Kind A.
...
- **Stundenplan-Fenster:** Untis liefert je nach Schule nur ein
  beschränktes Vergangenheits-/Zukunftsfenster (bei Schule −5/+9 Tage). Das
...
- **Markdown-Export-Button** ist in der Lovelace-Action eingebunden,
  ...
  `account: Kind B, subject: <Fach>` direkt aus den Entwicklertools auf.
