...
Sensoren (Beispiel für Anzeigename `Kind B`, Device-Slug
`untis_archive_kind_b`):

| Entity-ID | Inhalt |
|---|---|
| `calendar.untis_archive_kind_b_stundenplan` | Stundenplan mit Lehrstoff im Termin |
| `calendar.untis_archive_kind_b_ereignisse` | Fehltage (ganztägig) + Klassenarbeiten |
| `sensor.untis_archive_kind_b_fach_verlauf` | Lehrstoff-Historie gruppiert pro Fach |
| `sensor.untis_archive_kind_b_krankheitsperioden` | Krankheits-Perioden mit versäumtem Stoff |
| `sensor.untis_archive_kind_b_versaeumter_stoff` | Versäumter Stoff letzte 14 Tage |
| `sensor.untis_archive_kind_b_fehlzeiten_schuljahr` | Fehlzeiten-Zähler |
| `sensor.untis_archive_kind_b_stundenplan_aenderungen` | Vertretungs-/Ausfall-Zähler |
| `sensor.untis_archive_kind_b_lehrstoff_heute` | Lehrstoff der heutigen Stunden |

Heißen die Untis-Config-Entries anders, in `schul-cockpit.yaml` die
Slugs `untis_archive_kind_b` / `untis_archive_kind_a` durch deine
...
- Name: `Schul-Cockpit – Kind`
  ...
  Optionen: `Kind B`, `Kind A` — exakt wie die Anzeigenamen der
  Untis-Konten.
...
trigger:
  ...
  - platform: state
    entity_id:
      - sensor.untis_archive_kind_b_fach_verlauf
      - sensor.untis_archive_kind_a_fach_verlauf
...
- Oben in jeder View stehen zwei große Mushroom-Karten „Kind B" / „Kind A";
...
- **Übersicht**: Monatskalender rendert. Sichtbar sind
  ...
  - 📝 Klassenarbeiten (sofern die Schule sie in der Untis-API
    ausliefert – bei Schule noch nicht beobachtet, der Code zieht sie
...
- **Stundenplan-Fenster:** Untis liefert je nach Schule nur ein
  begrenztes Vergangenheits-/Zukunftsfenster (bei Schule −5/+9 Tage). Das
